...
def fetch_pas_gopc_avg_quota_data(lang: str = 'en') -> List[Dict]:
    ...
    """
    url = f"https://www.ha.org.hk/pas_gopc/pas_gopc_avg_quota_pdf/g0_9uo7a_p-{lang}.json"
...
def get_pas_gopc_avg_quota(lang: str = 'en', district: str = '') -> Dict:
    ...
    if district:
        data = [entry for entry in data if entry["District"].lower() == district.lower()]
    return {
        'data': data,
        'last_updated': datetime.now().isoformat(),
        'message': f"Retrieved data for {len(data)} clinics" + (f" in {district}" if district else "")
    }
